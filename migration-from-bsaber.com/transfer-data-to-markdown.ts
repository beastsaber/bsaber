...
const historicMotwData = JSON.parse(fs.readFileSync('./enriched-historic-motw-data.json', {encoding: 'utf-8'}));
for(const singleMotw of historicMotwData) {
    const sortedReviews = [...singleMotw.allReviews.filter(x => x.sentiment === 'POSITIVE')];
    sortedReviews.sort((a, b) => new Date(b.curatedAt ?? '2025-01-01').getTime() - new Date(a.curatedAt ?? '2025-01-01').getTime());
    ...
    fs.writeFileSync(`./src/collections/map-of-the-week/${new Date(singleMotw.publishedTime).toISOString()}.md`, createMarkdown(singleMotw), {encoding: 'utf-8'});
}